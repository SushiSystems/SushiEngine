...
namespace SushiEngine
{
    namespace Editor
    {
        ...
        using SushiEngine::Simulation::ISimulation;
        using SushiEngine::Simulation::IWorldEditor;
        using SushiEngine::Simulation::NULL_ENTITY;
        using SushiEngine::Simulation::ShapeParameters;

        PrefabThumbnailCache::PrefabThumbnailCache(SushiEngine::Render::IWindowRenderer& renderer,
                                                   ImGuiBackend& backend, Console& console)
            : renderer_(renderer)
            , prefab_renderer_(renderer.create_prefab_thumbnail_renderer())
        ...
        PrefabThumbnailCache::~PrefabThumbnailCache()
        {
        ...
        std::optional<ImTextureID> PrefabThumbnailCache::texture_for(
            const std::filesystem::path& path)
        {
            ...
            // every prior phase's failure policy exactly.
        ...
        void PrefabThumbnailCache::update()
        {
            ++frame_counter_;
        ...
        bool PrefabThumbnailCache::build_draws(
            const std::string& path, std::vector<SushiEngine::Render::PrefabThumbnailDraw>& out)
        {
            out.clear();
            ...
            try
            {
                ...
                std::unique_ptr<ISimulation> scratch = SushiEngine::Simulation::create_simulation();
                IWorldEditor& world = scratch->world();

                const EntityId root = Scene::apply_prefab(world, document, NULL_ENTITY);
                if (root == NULL_ENTITY)
                    return false;

                Scene::resolve_scene_assets(world, prefab_renderer_->asset_library());

                const std::size_t max_draws = prefab_renderer_->max_draws();
                for (EntityId id : world.entities())
                {
                    if (!world.has_shape(id))
                        continue;
                    const ShapeParameters shape = world.shape_parameters(id);
                    if (shape.mesh == SushiEngine::Render::INVALID_MESH)
                        continue;

                    if (out.size() >= max_draws)
                    {
                        // Over capacity is a load failure, not a partial thumbnail -- matching
                        // Phase 3a's own "more primitives than this renderer's capacity" policy.
                        out.clear();
                        return false;
                    }

                    const EntityTransform transform = world.world_transform(id);
                    SushiEngine::Render::PrefabThumbnailDraw draw;
                    draw.mesh = shape.mesh;
                    draw.material = world.material(id);
                    draw.model = SushiEngine::compose_transform(
                        transform.position, transform.rotation, transform.scale);
                    out.push_back(draw);
                }

                // Zero mesh-backed entities is also a load failure, not a blank-but-successful
                // render (see class docs / the design spec's error handling section).
                return !out.empty();
            }
        ...
        void PrefabThumbnailCache::upload_one(const std::string& path)
        {
            std::vector<SushiEngine::Render::PrefabThumbnailDraw> draws;
            SushiEngine::Render::FrameImage image;
            bool rendered = false;
            try
            {
                if (build_draws(path, draws))
                    rendered = prefab_renderer_->render_thumbnail(
                        draws.data(), draws.size(), THUMBNAIL_SIZE, THUMBNAIL_SIZE, image);
            }
            catch (const std::exception&)
            {
                rendered = false;
            ...
            if (!rendered)
            {
                // A build/render failure is permanent for this path, silently -- matching
                // ModelThumbnailCache's own policy: no log, tile keeps its Phase 1 glyph
                // forever.
                failed_.insert(path);
                return;
            }

            // render_thumbnail succeeding already loaded this prefab's mesh/material data into
            // the prefab renderer's isolated, no-removal-API asset stack -- the unbounded-growth
            ...
            ++prefabs_rendered_since_recreation_;
            if (prefabs_rendered_since_recreation_ >= PREFABS_PER_RENDERER_LIFETIME)
                recreate_prefab_renderer();
            ...
            try
            {
                ...
                if (vmaCreateImage(allocator_, &image_info, &alloc_info, &vk_image, &allocation,
                                   nullptr) != VK_SUCCESS)
                    throw std::runtime_error("vmaCreateImage(prefab thumbnail) failed");
                ...
                if (vkCreateImageView(device_, &view_info, nullptr, &view) != VK_SUCCESS)
                    throw std::runtime_error("vkCreateImageView(prefab thumbnail) failed");
                ...
                if (vmaCreateBuffer(allocator_, &staging_info, &staging_alloc_info, &staging,
                    ...
                    throw std::runtime_error("vmaCreateBuffer(prefab thumbnail staging) failed");
                ...
                if (vkCreateCommandPool(device_, &pool_info, nullptr, &command_pool) !=
                    VK_SUCCESS)
                    throw std::runtime_error("vkCreateCommandPool(prefab thumbnail) failed");
                ...
                if (vkAllocateCommandBuffers(device_, &command_alloc, &command) != VK_SUCCESS)
                    throw std::runtime_error("vkAllocateCommandBuffers(prefab thumbnail) failed");

                VkCommandBufferBeginInfo begin{};
                begin.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
                begin.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
                if (vkBeginCommandBuffer(command, &begin) != VK_SUCCESS)
                    throw std::runtime_error("vkBeginCommandBuffer(prefab thumbnail) failed");
                ...
                if (vkEndCommandBuffer(command) != VK_SUCCESS)
                    throw std::runtime_error("vkEndCommandBuffer(prefab thumbnail) failed");

                VkFenceCreateInfo fence_info{};
                fence_info.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
                if (vkCreateFence(device_, &fence_info, nullptr, &fence) != VK_SUCCESS)
                    throw std::runtime_error("vkCreateFence(prefab thumbnail) failed");
                ...
                if (vkQueueSubmit2(queue, 1, &submit, fence) != VK_SUCCESS)
                    throw std::runtime_error("vkQueueSubmit2(prefab thumbnail) failed");
            ...
            catch (const std::exception& error)
            {
                console_.append(LogLevel::Warning,
                                "Prefab thumbnail upload failed for '" + path + "': " +
                                    error.what());
        ...
        void PrefabThumbnailCache::recreate_prefab_renderer()
        {
            ...
            prefab_renderer_.reset();
            prefab_renderer_ = renderer_.create_prefab_thumbnail_renderer();
            prefabs_rendered_since_recreation_ = 0;
        }

        void PrefabThumbnailCache::destroy_thumbnail(ResidentThumbnail& thumbnail)
        {
