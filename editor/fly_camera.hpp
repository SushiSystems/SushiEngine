...
namespace sushi::editor
{
    ...
    struct FlyCamera
    {
        SushiEngine::Vec3 position{0.0f, 2.5f, 7.0f};
        ...
        SushiEngine::Vec3 forward() const noexcept
        {
            const float cos_pitch = std::cos(pitch_radians);
            return SushiEngine::normalize(SushiEngine::Vec3{
        ...
        SushiEngine::Vec3 right() const noexcept
        {
        ...
        SushiEngine::Mat4 view_matrix() const noexcept
        {
            return SushiEngine::look_at(position, position + forward(),
                                        SushiEngine::Vec3{0.0f, 1.0f, 0.0f});
        }
