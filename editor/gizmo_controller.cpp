...
namespace sushi::editor
{
    ...
    namespace
    {
        ...
        bool project_to_screen(const SushiEngine::Mat4& view_projection, const Vec3& point,
        ...
        Vec3 rotate_vector(const SushiEngine::Quat& q, const Vec3& v)
        {
            const Vec3 qv{q.x, q.y, q.z};
            const Vec3 t = cross(qv, v) * SushiEngine::Scalar(2);
                          ...
                          const ImVec2& screen, const ImVec2& origin, float width, float height)
        {
            ...
            const Vec3 eye = right * static_cast<Scalar>(-view.m[12]) +
            ...
            const Vec3 direction = SushiEngine::normalize(
        ...
        bool intersect_plane(const Ray& ray, const Vec3& point, const Vec3& normal, Vec3& out)
        {
            using Scalar = SushiEngine::Scalar;
    ...
    GizmoController::Result GizmoController::manipulate(
        GizmoMode mode, GizmoSpace space, SushiEngine::sim::EntityTransform& transform,
        ...
        float width, float height, bool hovered, const GizmoSnap& snap)
    {
        ...
        const Vec3 pivot = transform.position;
        ...
        const Vec3 draw_axes[3] = {use_local ? rotate_vector(transform.rotation, AXES[0]) : AXES[0],
        ...
        if (mode == GizmoMode::Rotate)
        {
            ...
            if (origin_ok && axis_ < 0)
                for (int a = 0; a < 3; ++a)
                {
                    ...
                    for (int s = 0; s <= SEGMENTS; ++s)
                    {
                        ...
                        const Vec3 p = pivot + draw_axes[perp_a[a]] * (std::cos(t) * ring_radius) +
            ...
            if (axis_ >= 0)
            {
                ...
                {
                    const Vec3 current_vector = SushiEngine::normalize(hit - pivot);
                    ...
                    const SushiEngine::Quat spin = SushiEngine::quat_axis_angle(
            ...
            else if (origin_ok && hovered && left_clicked)
            {
                ...
                if (best >= 0)
                {
                    const Vec3 grab_axis = draw_axes[best];
                    const Ray ray = screen_to_ray(camera_view.view, camera_view.projection, mouse,
                                                  image_origin, width, height);
                    Vec3 hit;
        ...
        if (origin_ok)
            for (int a = 0; a < 3; ++a)
            {
                const Vec3 tip = pivot + draw_axes[a] * handle_length;
                tip_ok[a] =
        ...
        if (axis_ >= 0)
        {
            if (axis_ == 3)
            {
                ...
                if (factor < 0.01f)
                    factor = 0.01f;
                Vec3 s = start_transform_.scale * static_cast<SushiEngine::Scalar>(factor);
                if (snap.enabled)
                    s = Vec3{snap_to(static_cast<float>(s.x), snap.scale),
