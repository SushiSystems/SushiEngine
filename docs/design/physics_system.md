...
**Status:** in progress, P0 to P7 and PX complete, P8 under way, P9 four-fifths done — kinematic
bodies, the character controller, the event sink and the rigid core's rollback snapshot landed
2026-08-07 to 08 (P9-A to P9-D, §16.46 to §16.49); the networking harness is not started, and the
snapshot's deformable half is a named follow-up (§16). P1, P2, P6
...
### 16.46 P9-A: kinematic bodies, and the branch that was missing between two others

`BodyFlags::kinematic` had existed since P0
(`engine/domain/physics/include/SushiEngine/physics/core/body_flags.hpp`) and **nothing read it.**
Nothing wrote it either: `extract_rigid_bodies`
(`engine/world/simulation/include/SushiEngine/simulation/physics_extract.hpp`) filled a
description's pose, inverse mass, inverse inertia and drag and never touched the flag word, so
every body reached the solver as `dynamic_body`. An author who found the bit and reached for
`inv_mass = 0` instead got a *pinned* body, which pushes nothing — a different thing wearing the
same number.

**Three of the four mechanisms a kinematic body needs were already built, for other reasons.**
Immovability: every projection in
`engine/domain/physics/include/SushiEngine/physics/core/rigid_body.hpp` scales its correction by
the body's inverse mass, so a body at zero takes no share of any contact, constraint or joint —
no kinematic case, anywhere. Pushing: a kinematic body pushes by *moving*, its pose advancing each
sub-step until the next sub-step's contact projection sees the overlap and depenetrates the other
side. Island isolation: `IslandBuilder::conducts`
(`engine/domain/physics/include/SushiEngine/physics/scene/islands.hpp`) requires a positive
inverse mass, so a conveyor cannot hold a whole scene awake through a chain of contacts — the
behaviour PhysX implements deliberately, here falling out of a rule written for static bodies.

**What was missing was one branch.** `predict` guarded the acceleration accumulation and the
position integration with the same `inv_mass > 0` test, which is correct for the two body kinds
that existed and wrong for the one between them: a kinematic body must skip the acceleration and
still perform the integration, and no single inverse-mass test says both. It now has its own
branch, taken before either, integrating pose and orientation from the commanded velocity with no
gravity, no field and no drag. `update_velocity` skips it for the opposite reason — the velocity
is the input, not the output, and re-deriving it would bleed a commanded spin away through the
logarithmic map's small-angle approximation every sub-step. That map is now
`angular_velocity_between`, lifted out so the commanded turn and the recovered one cannot come
from two hand-written copies.

`is_simulated` is deliberately **unchanged**. Its question is "is this body integrated and
projected this tick", and for a kinematic body both are yes; twenty call sites ask it, and
narrowing it would have dropped kinematic bodies out of the contact list, the statistics and the
wake logic in one edit.

**The command is a target pose, not a velocity.** `IRigidBodyService::move_rigid_body`
(`engine/world/simulation/include/SushiEngine/simulation/physics_services.hpp`) records where the
body should be by the tick's end, and the tick divides that difference by its own duration —
which is the first moment the duration is known, since the command arrives before the tick starts.
Chosen over a velocity setter because an author driving a platform from an animation clip has a
pose and would otherwise have to differentiate it, because a re-derived velocity cannot accumulate
positional drift, and because the derived velocity is exactly what a contact's friction term
reads, so a crate on a rising platform is carried without a line of code that knows it is standing
there. A kinematic body given no target has its velocity cleared: nothing in the tick would slow
it otherwise, so driving one means writing its pose every tick — which a clip or a script loop
does anyway.

**The invariant is established at the boundary.** The extract writes the flag *after* the density
derivation and forces inverse mass and inertia to zero, so ticking the box wins over whatever mass
the collider would have implied, and the solver never has to hold a second opinion about what a
kinematic body weighs.

**One consequence needed real work.** Because a kinematic body does not conduct, a crate resting on
a platform forms its own island and sleeps — and a sleeping body is skipped by `predict` outright,
so it would hang in the air while the platform left. Waking the platform's island wakes nothing but
the platform. So `move_rigid_body` walks the contact records instead of the partition and wakes
every body sharing one with the moved slot. Those records are last tick's, the same staleness
sleeping already tolerates, and the cost is bounded by the moved body's own contact count rather
than by scene size.

**Reachable without writing C++**, which is what PX established as the standard: a Kinematic
checkbox at the top of the Rigid Body inspector, greying out the mass rows it makes meaningless;
the field round-tripped through the scene file; and a transform edit routed on the flag, so
dragging a gizmo *places* a dynamic body and *moves* a kinematic one. There is no second API to
learn — driving a platform is writing its transform. The serialization pass also found that
`PhysicsBodyParameters::density` had never been written to a scene file at all: harmless to the
simulation, since the numbers it derives were saved, but the author's density box came back empty
with the derived figures presented as hand-typed.

**Six tests**, five through the scene seam in `tests/integration/test_physics_simulation.cpp` and
one in `tests/integration/test_solver_conformance.cpp`. The conformance scene asserts the absolute
position as well as the agreement, because two solvers that both lost the branch would agree with
each other perfectly, on the floor. **All six run on the CPU backend: P9-A needed no GPU**, which
makes it the first phase row in some time whose acceptance is not waiting on hardware.
...
### 16.49 P9-D: the snapshot, and the two bytes nobody owns

P9-D delivers no feature anyone can see. It delivers a proof: capture at tick K, run on to N,
restore K, replay to N, and the state at N is byte-identical to the state the uninterrupted run
reached. `IPhysicsSnapshotService` on `IPhysicsScene` is the whole surface, and
`engine/world/simulation/include/SushiEngine/simulation/physics_snapshot.hpp` is the bounds-checked
cursor underneath it.

**The rule that decides what goes in is one question: does the next tick read this, or rebuild it?**
Body columns, joint state, the island partition and the tick's own scalars are read. The
broadphase, the query index, the contact proxies and this tick's event lists are rebuilt from dirty
flags, so restore marks them stale rather than carrying them — a stale derived index that disagrees
with the state it was derived from fails in a way nothing traces back to a snapshot.

**The contact records are the load-bearing part, and they do not look it.** `current_` and
`previous_` read as bookkeeping and are where **warm starting lives**: a persisting contact carries
its accumulated impulse forward, which is what lets a stack settle instead of sinking and
rebounding. A snapshot without them restores a simulation identical in every pose that converges
differently on the very next tick — and a pose comparison, which is what one would naturally write
first, passes. §12.3 lists "the warm-start accumulators" without saying where they are. They are
here.

**Two padding hazards had to be closed, and they are the reason this row is subtler than it looks.**
`ContactRecord` and `Island` are memcpy'd wholesale into the blob, and both were default-initialized
rather than value-initialized — so their padding bytes were indeterminate, and one logical state
could produce two different blobs. Byte equality is a claim about *bytes*, so the bytes have to be a
function of the simulation and nothing else. The same reasoning is why each sink's cooldown table is
written sorted by pair: an `unordered_map`'s iteration order is not a property of the simulation,
and §12.1's first rule already says so about body order.

**The control test is not a formality.** Nothing in this codebase had ever asserted byte equality of
physics state — `tests/integration/test_solver_conformance.cpp` compares the host and device solvers
at a 1e-9 tolerance, which is a different and weaker claim. So the first test runs two identical
scenes side by side for two hundred ticks and requires their blobs to match at every one. Without
it, a failing rollback assertion could not distinguish a lossy snapshot from a simulation that was
never deterministic, and the wrong half would get investigated.

**Six tests**: the control, a rollback across a settling stack, one across a sleeping stack, the
refusal of a foreign body set and of a truncated blob, and the row's own ten thousand ticks. All on
the CPU backend. **P9-D needed no GPU, which makes it the only item of this row whose acceptance
number could be produced on the machine it was written on.**

**Scope, stated rather than implied.** This is the **rigid core**: bodies, joints, contacts,
islands, scalars. Soft bodies and their plastic rest matrices, fracture, beams, elements, vehicles
and cloth are not captured, so the acceptance clause's "including contacts" is met and "and
fracture" is not. Taking the rigid core first was deliberate: a ten-thousand-tick byte comparison
that fails tells you *that* something diverged, and capturing six subsystems at once would make the
first question "which one" rather than "where". Restoring across a spawn or a despawn is also out —
it needs the solver's handle table rewound, and a blob whose body set differs is refused outright
rather than half-applied. Both are rows in `docs/design/remaining_work.md`.
