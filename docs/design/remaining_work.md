...
| Stale de-abbreviated identifiers | 9 sites, 4 documents | `QualityParams` for the real `QualityParameters`: three sites in `docs/design/animation_system.md`, one in `docs/design/atmosphere_system.md`, three in `docs/design/editor_ux_overhaul.md` and two in `docs/design/vfx_particle_system.md`. The restructure's de-abbreviation pass renamed the type and the corpus kept the old name. No check sees this: the citation checker matches a path, and an identifier is not a path. The two changelog occurrences are historical and stay. Needs a sweep for every identifier that pass renamed, not only this one. |
