...
Three states appear, and they are different kinds of work:

- **Open** — the owning document is accurate and the work is not built. Most of the backlog.
- **Withdrawn claim** — the document claimed the work as delivered, the tree did not carry it, and
  the claim has been withdrawn. The work itself is still owed.
- **Unmeasured** — the code exists and is reachable from a build target, and a numeric or visual
  acceptance criterion has never been checked because no machine here has the hardware. These close
  with a measurement, not with code, which is why they are a separate table.

A phase the audit found claimed and true appears nowhere below. Absence from this file means the
phase is done, so a phase that goes missing from it reads as finished — which is why coverage
matters more here than concision.

## Open

| Phase | Document | State | What it still needs |
| --- | --- | --- | --- |
| D | [atmosphere_system.md](atmosphere_system.md) | open, blocked | Terrain and surface coupling: orographic lift, rain shadow, föhn, valley fog, sea and lake breeze, terrain-driven turbulence, land-cover convective initiation. §15 records a blocker that needs restating before the phase can be scoped. |
...
| F | [atmosphere_system.md](atmosphere_system.md) | open, optional | A microscale large-eddy-simulation domain of roughly 20 kilometres at 100 metres. The document says "possibly never"; unscoped until someone asks for it. |
...
| RHI1–RHI11 | [cross_platform_engineering_plan.md](cross_platform_engineering_plan.md) | not started | The neutral `Rhi::` vocabulary through Metal parity and the Slang migration. RHI1 is next per §10 and is unblocked: RHI0's golden-image harness is closed. |
| RUNTIME-PORT1–8 | [cross_platform_engineering_plan.md](cross_platform_engineering_plan.md) | not started | The native execution backend beyond its current Windows and SushiRuntime seam, and every non-Windows bring-up. RUNTIME-PORT1, the native backend on Linux first, is next. |
| PLATFORM1–9 | [cross_platform_engineering_plan.md](cross_platform_engineering_plan.md) | not started | Linux parity, then mobile and console bring-up. PLATFORM0 gave all of them the editor and runtime split they depend on. |
| Deferred items | [editor_feature_sync_gaps.md](editor_feature_sync_gaps.md) | transferred | All six deferred items moved intact into [editor_ux_overhaul.md](editor_ux_overhaul.md) §2.4's verdict table. Whether §2.4's own wire-or-remove verdicts were then carried out was outside the audit's scope, so this row records a handover rather than a closure. |
...
| P9 | [physics_system.md](physics_system.md) | not started | Kinematic bodies, a character controller, `IPhysicsEventSink` and its fan-out to gameplay, audio and VFX, rollback snapshot integration, and the networking validation harness with its 10,000-tick byte-equality test. |
...
| P2 | [prefab_system.md](prefab_system.md) | not started | Override resolution. `prefab_entity_id` is written by P1 and has no consumer anywhere; giving it one is the next design-and-build task in this line. |
| P3 | [prefab_system.md](prefab_system.md) | not started | Nested prefabs and a prefab edit mode. Designed only as far as their boundary. |
| P4 | [prefab_system.md](prefab_system.md) | not started | A runtime instantiation API. Prefab instantiation is editor-only today; nothing in the deterministic loop references it. |
| 4 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open refinements | Area and IES lights, projected normal-map decals, shadow caching, screen-coverage quadtree tiles, and adaptive per-light percentage-closer soft shadows. The clustered core is shipped; these are the tier-scalable remainder. |
| 5 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open refinement | Local reflection probes (§5.4), deferred pending the editor's scene-capture visual loop. |
| 6 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open refinements | Multiple cascades, emissive injection, toroidal amortization, and the Tier B ray-query tracer, the last of which needs ray-tracing hardware. |
| 7 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open refinements | Cascaded-shadow-map god-ray shafts, punctual-light fog, per-tier lookup-table resolutions, and temporal amortization. |
| 9 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open deferral | HDR10 and scRGB output. No swapchain colour-space code exists; verifying it afterwards also needs an HDR display. |
| 10 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open deferral | Sparse virtual texturing, deferred with a stated rationale and absent from the tree. |
| 11 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open, blocked | The runtime-side interop import, blocked on a SushiRuntime API that does not exist. The same blocker is why `create_interop_buffer` has no caller in the restructure's phase-3 inventory. |
| 12 | [render_pipeline_refactor.md](render_pipeline_refactor.md) | open, blocked | 12.1, 12.2 and 12.3 Tier B all need `VK_KHR_ray_query`, which the Pascal development device does not have. 12.2 additionally needs the NRD software development kit and the licensing decision that goes with it. |
| 4 | [repository_restructure.md](repository_restructure.md) | open | The structural half is done and the citation half was repaired against the corpus checker. What phase 4 still owes is the three unswept classes in "Backlog this audit created" below, which the repair pass measured but did not close. |
...
| Phase | Document | State | What it still needs |
| --- | --- | --- | --- |
| §5 rule 11 / §13 | [model_import.md](model_import.md) | withdrawn claim | Collider generation on import. The setting flows into the plan as a per-entity flag and `engine/world/model_import/source/prefab_output.cpp` never reads it, so no collider is ever created. Needs a case in that switch calling the physics cooking path, and a test proving a model imported with the setting on carries a collider. |
| M1 | [SUSHILOOP.md](SUSHILOOP.md) | withdrawn claim | Continuous integration on both platforms. `.github/workflows/ci.yml` defines Linux jobs only. Needs a Windows job, or the claim stays withdrawn and Windows stays a local build. |
| M3 | [SUSHILOOP.md](SUSHILOOP.md) | withdrawn claim | Delta snapshot recording. `engine/world/loop/include/SushiEngine/loop/rollback.hpp` copies every live chunk in full every tick, as its own header comment says. Needs the per-write dirty tracking that comment names as the follow-on. |
| M4 | [SUSHILOOP.md](SUSHILOOP.md) | withdrawn claim | Real sockets. `engine/world/loop/include/SushiEngine/loop/net.hpp` carries a loopback transport and states it has no sockets, threads or serialization. Needs a real transport behind the same seam the loopback already implements. |
...
| §12.5 device evaluator parity | [animation_system.md](animation_system.md) | unmeasured | The one animation item needing SushiRuntime's device path rather than a display. Needs a test running the SYCL kernel and asserting host and device agree, then a run. |
...
The corpus-wide citation audit that produced this file repaired 1,074 citation sites and left three
...
standard, and they are recorded here rather than absorbed into it because an unmeasured population
that nobody wrote down is exactly the failure this audit exists to correct.
...
| Unverified line numbers | 34 citations | Citations that already resolved were exempted from every check, so their line numbers were never verified. `docs/design/static_mesh_authoring.md` cites `applications/editor/source/main.cpp:489-501` for a copy that ends at line 488. Needs a pass that checks a line locator against the file it names. |
