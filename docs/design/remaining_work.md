...
| P9-B follow-up: pushing | [physics_system.md](physics_system.md) | open | A character stops at a dynamic body and does not push it, because its sweep halts a skin width short and leaves no overlap for the contact projection. Needs the mover to report its blocking hits and the scene to spend an impulse per hit. Pinned by `ACharacterStopsAtADynamicCrateAndDoesNotPushIt`, which must be rewritten rather than loosened. |
| P9-B follow-up: platform riding | [physics_system.md](physics_system.md) | open | A character standing on a kinematic platform is not carried by it: both bodies have zero inverse mass, so the contact resolves to nothing. Needs the caller to be given the platform's own delta, or the controller to track what it is standing on. Pinned by `ACharacterOnAMovingPlatformIsNotCarriedByIt`. |
| P9-C follow-up: demo scene | [physics_system.md](physics_system.md) | open | The scene the row's acceptance names. The sink and the impact listener landed 2026-08-07 (§16.48); the scene needs an impact sound and a spark material, and one built from silent, invisible placeholders would close the clause in name only. An asset task, not an engine one. |
| P9-C follow-up: particle burst | [physics_system.md](physics_system.md) | open | `ParticleEmitterParameters` carries only `seed` and `playing`, so there is no burst in the emitter model to ask for and `ImpactResponseListener` fakes one by stopping the emitter after a fixed window. The emitter model is where a burst belongs. |
| Audio `looping` is dropped at the boundary | [audio_system.md](audio_system.md) | open | `AudioEmitterParameters::looping` is authored and serialized and reaches nothing: `engine/world/simulation/include/SushiEngine/simulation/audio_extract.hpp` copies thirteen emitter fields and not that one, and `EmitterSnapshot` has no such field, because whether a sound loops is decided by the asset its `VoiceSource` is built from. Found 2026-08-07 while wiring impact audio. Either bind it or remove it — the choice is a design decision about who owns looping. |
| P9-D follow-up: the deformable half | [physics_system.md](physics_system.md) | open | The snapshot captures the rigid core only (§16.49). Soft-body vertex state, plastic rest matrices, fracture, beams, elements, vehicles and cloth are not in it, so the row's "including contacts and fracture" clause is half met. The cursor and the capture/restore shape exist; this is extending them, subsystem by subsystem so a divergence stays findable. |
| P9-D follow-up: rollback across a spawn | [physics_system.md](physics_system.md) | open | `restore_snapshot` refuses a blob whose body set differs from the live one. Rewinding across a spawn or a despawn needs the solver's handle table rewound too; applying a snapshot over a different set would leave bodies holding another body's state. Refusing loudly is correct until that exists. |
| P9-E | [physics_system.md](physics_system.md) | blocked | The networking validation harness. Needs [SUSHILOOP.md](SUSHILOOP.md)'s M4 real sockets, a clause that document withdrew in this audit. |
...
| P2 follow-up: Inspector affordances | [prefab_system.md](prefab_system.md) | open | Override resolution landed 2026-08-08 (§11); what is left is showing a component as overridden and reverting it. The diff that would drive both is what a refresh already computes, so this is editor-only work. |
...
| PROF2–PROF4 | [profiling_system.md](profiling_system.md) | open | Renderer counters and memory budgets behind defaulted `ISceneView` accessors, the editor-owned system metrics provider, and the polish pass. PROF0's Profiler panel and PROF1's `engine/foundation/profiling` module and instrumented editor loop are built. |
