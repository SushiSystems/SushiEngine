...
| P9-C | [physics_system.md](physics_system.md) | open | `IPhysicsEventSink` and its fan-out to gameplay, audio and VFX, plus the demo scene that is half the row's acceptance. The stream underneath it is built — `ContactEvent` carries phase, point, normal and impulse, and reaches `ISimulation::physics_contacts` — and has no consumer outside the tests. |
