...
**Status:** superseded, in part by `docs/design/editor_ux_overhaul.md` §2.4; its "Fixed this pass"
log stands.
...
- **GTAO** (`RenderSettings::gtao`) had no editor UI despite being fully wired through
  `engine/presentation/render/source/passes/gtao_pass.cpp` and round-tripped in
  `engine/world/authoring/source/preferences.cpp`. Added a section to the Rendering panel
...
- **Ray-traced sun shadows** (`ShadowSettings::ray_traced`) and **secondary directional shadow
  casters** (`max_directional_shadow_casters`) were consumed by
  `engine/presentation/render/source/passes/ray_traced_shadow_pass.cpp` /
  `engine/presentation/render/source/frame/quality.cpp` but had no toggle in the Shadows section.
  Added both.
...
- **Blend shapes / morph targets**: `AnimatedMeshPreview::set_morph_weights` was a dead seam — glTF
  morph-target import already existed in
  `engine/presentation/render/source/material/gltf_importer.cpp` (`primitive.targets[]` →
  ...
  (`applications/editor/source/animation/animator_preview_panel.cpp`). Still manual per-target
  weights, not clip-driven (design `docs/design/animation_system.md` §12.1/§12.2 — glTF `WEIGHTS`
  animation-channel import doesn't exist yet; that's a separate, larger piece of work).

## Deferred — no-op controls (UI exists, nothing reads it)

- **Material Inspector**: "Receive Shadows" and "GPU Instancing" checkboxes
  (`applications/editor/source/ui/material_inspector.cpp`) toggle `Material::receive_shadows` /
  `gpu_instancing` (`engine/domain/material/include/SushiEngine/material/material.hpp`), but no
  render pass reads either field — every material appears to support per-object shadow-receiving and
  instancing control, and toggling either does nothing. Fixing this for real means either:
  - wiring `receive_shadows` into whichever shadow-sampling shader/pass sees this material
    (`engine/presentation/render/shaders/pbr.frag` and friends), and `gpu_instancing` into an actual
    instancing batcher, or
...
- **GPU Culling panel**: "Freeze frustum (debug)" and "Show statistics"
  (`applications/editor/source/ui/editor_panels.cpp`, `draw_gpu_culling_panel`) toggle
  `GpuCullingSettings::freeze` / `show_statistics`, which
  `engine/presentation/render/source/passes/cull_pass.cpp` never reads. The struct's own doc comment
  already says these are "reserved for the editor's cull debug view" — i.e. this was always meant to
  be finished later, not a regression. Needs: a frozen-frustum debug mode in
  `engine/presentation/render/source/passes/cull_pass.cpp`, and a readback path for per-frame cull
  counts (the "Show statistics" branch currently just points at the Profiler HUD instead of
  producing anything itself).

## Deferred — engine feature has no editor UI at all

- **Audio acoustic geometry**:
  `engine/domain/audio/include/SushiEngine/audio/acoustic_geometry.hpp`,
  `engine/domain/audio/include/SushiEngine/audio/acoustic_raytracer.hpp`,
  `engine/domain/audio/include/SushiEngine/audio/occlusion.hpp`,
  `engine/domain/audio/include/SushiEngine/audio/portals.hpp`,
  `engine/domain/audio/include/SushiEngine/audio/propagation.hpp`,
  `engine/domain/audio/include/SushiEngine/audio/early_reflections.hpp`,
  `engine/domain/audio/include/SushiEngine/audio/convolution_reverb.hpp` are all real, but
  `applications/editor/source/audio/audio_panels.cpp` /
  `applications/editor/source/audio/audio_authoring_panel.cpp` only expose the mixer, per-entity
...
- **VFX `SortMode`, `SimulationDomain`, `BeamModule`**
  (`engine/domain/vfx/include/SushiEngine/vfx/modules.hpp`) have no widgets in
  `draw_particle_system_component` — only Spawn/Shape/Init/Gravity/Drag/
  ...
  what `engine/domain/vfx/include/SushiEngine/vfx/modules.hpp` defines.
