...
- Shader pipeline is GLSL-only, compiled offline to SPIR-V via a glslang host tool that emits C++ headers with embedded SPIR-V words. Zero cross-compilation exists. This was an **explicit, deliberate, previously-deferred decision** (`docs/design/render_pipeline_refactor.md` Phase 3.6), with a stated revisit trigger: *"when a non-SPIR-V backend is required."* That trigger has now fired.
