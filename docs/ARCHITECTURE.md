...
that slots in behind the same encode → decode seam.
...
(I3DL2 Reflections/ReflectionsDelay) are on the parameter set but await the acoustic BVH at S7.
...
a real ECS world (listener-local conversion, zone containment, the read-only invariant). `Room` /
`Portal` and the I3DL2 early reflections wait for the S7 acoustic BVH.

The roadmap continues S7 (occlusion / rooms + portals + acoustic BVH) through S10; S0–S4 (the
critical path) plus S5 (reverb) and S6 (ECS integration) are complete.
