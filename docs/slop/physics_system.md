...
| `IPhysicsSimulation` split (§4.3) | **Not started.** `IConstraintSolver` is in place, but the `sim/` boundary is untouched and `RuntimeSimulation` still gathers descriptors inline. |
| Deterministic contact ordering (§12.1) | **Not started.** Contacts are still resolved in a host pass outside the solve graph; ordering them is part of moving them in. |
| Segmented accumulation (§12.2) | **Not started.** `add_segmented_reduce` is not yet a call site, because the per-body contact-impulse accumulation it serves does not exist until contacts move into the graph. |
| `PhysicsStatistics` wiring (§13.3) | **Partial.** The counters are populated by the solver and asserted by tests; nothing surfaces them in the editor yet, and the per-stage timings are a single total rather than a breakdown. |
