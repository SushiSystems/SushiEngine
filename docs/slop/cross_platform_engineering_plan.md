...
Status: research complete, no implementation started. Ground-truth findings verified against the source tree on 2026-07-25. PS3 is explicitly out of scope (EOL hardware, no active toolchain, no serious studio still targets it). PS5/PS4 sections are written entirely from public knowledge — this project has no PlayStation Partners registration or devkit access as of this writing, so nothing NDA'd is claimed or guessed at anywhere in this document.
...
This priority is deliberately expressed as a **sequencing decision, not a structural one**: no wall, interface, or milestone in §4–§6 is platform-ordered in its design — `Exec`, the RHI, and `IPlatformHost`/`IDisplaySurface` are all written so a platform can be slotted in through an existing seam (compile-time backend policy, backend registry, capability-flag query) whenever its business/technical prerequisites clear, independent of where it sits in this priority list. That is intentional: the priority order can change again without requiring a re-architecture, which is the whole point of building the seams this way rather than hardcoding a platform list into any interface.
...
|---|---|---|---|---|
| 1 | **SushiRuntime / SYCL execution wall** | The ECS's `World`/`Schedule`/`Chunk` types are typed directly against `SushiRuntime::API::{Runtime,Graph,Buffer}`. SushiRuntime's SYCL toolchains (intel-llvm, AdaptiveCpp, oneAPI DPC++) have **zero path** to macOS, iOS, Android, or PlayStation — no code, no docs, no CI, not even listed as a non-goal. Any TU that includes `World`/`Schedule` cannot currently be compiled for 5 of 7 target platforms. | Introduce a **compile-time execution-backend seam** (`SushiEngine::Exec`) inside SushiEngine, with the existing SushiRuntime path as one implementation and a new native thread-pool/DAG implementation as the other. This is smaller than it sounds: the actual SYCL surface touched by ECS code is **12 symbol usages across 5 files** — the system bodies themselves are already plain C++. | §4 |
| 2 | **Vulkan-hardcoded RHI wall** | The renderer's outer seam (`IRenderDevice`, `IWindowRenderer`, `ISceneView`) is genuinely backend-neutral, but everything beneath it — the render graph, all 39 passes, the lighting/scene/resource systems — is typed directly against `Vk*`/`VmaAllocation`. 69 files include `<vulkan/vulkan.h>`; 26 include `<vk_mem_alloc.h>`. No second backend exists anywhere. | A layered `Rhi::{types, handles, ICommandList, IDevice}` abstraction, introduced behind a golden-image + command-trace regression harness (there are currently **zero render tests** — a two-pixel probe is the entire safety net for a 39,894-line refactor). Metal is the real second backend; MoltenVK is a bring-up crutch and permanent differential oracle, not the shipping answer. | §5 |
...
- Every hand-barriered compute pass (13 of them) declares **zero attachments** — no pass mixes rendering and manual barriers in a way that would be illegal on Metal's encoder model.
...
- Exactly two working POSIX code paths exist (`editor/core/preferences.cpp`, `editor/main.cpp`'s `default_projects_root()`); two functions are explicitly Windows-only with a no-op stub and an in-code comment: *"Windows-only for now; the project targets Windows first."*
...
- `render/` is 39,894 LOC across `graph/` (2,370), `passes/` (14,587, 39 passes), `scene/` (4,337), `rhi/vulkan/` (4,140, the only backend), `resources/` (3,428), `shaders/` (10,484, ~90 GLSL files).
...
- GPU data binding uses three simultaneous strategies: a 32-binding push-descriptor set 0 (**already documented as full** — the next frame-global resource has nowhere to go), a bindless `update-after-bind` heap (set 1), and 128-byte push constants. All Vulkan-typed, no abstraction.
...
- Confirmed real, implemented subsystems: a SYCL task graph with a lock-free sharded dependency tracker (RAW/WAR/WAW hazard detection over byte-range `ResourceRegion`s), a NUMA-aware USM allocator (hwloc-backed), and hardware discovery.
...
- **This gap is not documented anywhere** — not in code, not in CMake, not in the project's own TODO/roadmap docs, not even as a stated non-goal. The only platform branching in the entire `include/`+`src/` tree is `_WIN32` vs. POSIX thread-affinity code.
- SushiRuntime's own CI runs exclusively on `ubuntu-latest`.
- **However**, the actual coupling from SushiEngine's ECS into this machinery is narrow: `Schedule::each`'s system kernels contain **no SYCL** — they are plain C++ over raw SoA column pointers. The only thing that makes a system "device code" is a wrapper inside SushiRuntime itself (`Graph::emit_dynamic_per_element`). SushiEngine's own SYCL usage is 12 symbol usages across 5 files, one of which (`audio/accelerator_sycl.hpp`) is already correctly isolated behind an `IDspAccelerator` interface.
...
1. **The allocator/interop seam.** SushiEngine's shared-USM component storage (`Chunk::Column::data`) is what makes simulation output directly renderable without a staging upload (`render/interop.hpp`). On a platform with no SushiRuntime, the native `Exec` backend's allocator must still be able to hand out graphics-visible memory — this requires the `Exec` allocator (Wall 1) and the RHI's texture/buffer creation (Wall 2) to agree on an injectable-allocator hook. Design this once, explicitly (see §4.6 / §5's binding-model notes); do not let it get "figured out later," because retrofitting it means reallocating every GPU resource in the engine.
2. **Android and iOS bring-up require all three walls to have landed their platform-specific milestone before a scene can render on-device.** Desktop (Windows/Linux) requires none of this — it works today. macOS needs Walls 1 and 2 (native Exec backend + Metal-or-MoltenVK) but not a new Wall-3 windowing backend beyond what SDL2 already provides for desktop Apple.
...
## 4. Wall 1 — SushiRuntime execution portability (`SushiEngine::Exec`)
...
| **B — Genuine GPU compute** | Device-side kernels: PGS/XPBD solvers, batch animation, particle sim | 2 physics solvers + 1 animation evaluator (audio's SYCL accelerator is already optional-by-default) | No — this is the fidelity tier. Batch animation already has a CPU twin (`animation/batch_evaluator.hpp`). |
...
New namespace `SushiEngine::Exec`, header tree `include/SushiEngine/exec/`. Mirrors three precedents already in the codebase (`audio::IDspAccelerator`, `sim::ISimulation`, `render::rhi::IDevice`) — idiomatic, not novel.
...
- **Backend selection is a CMake-level compile-time policy** (`SUSHI_EXEC_BACKEND=runtime|native`), not a vtable.

Core vocabulary types: `Exec::ResourceRegion` (mirrors SushiRuntime's `Core::ResourceRegion` structurally, for a trivial adapter conversion), `Exec::NodeDesc` (name, reads/writes, capacity, live-count provider, enabled provider), `Exec::RunReport`, `Exec::Graph` (duck-typed — both backends implement the same surface without a common base for the templated methods), `Exec::Context` (`create_graph()`, `allocate<T>(count, MemoryVisibility)`, `capabilities()`), and `Exec::Buffer<T>`.
...
**Native backend internals** (ships as a compiled `sushi_exec_native` static library, not header-only — a thread pool doesn't belong in headers): linear-time (not O(N²)) DAG compilation via last-writer/reader-set tracking per resource region (mirror SushiRuntime's `dependency_tracker.cpp` as the behavioral spec — a knowledge dependency, not a build dependency, so the one-way `SushiEngine → SushiRuntime` rule stays intact); a Chase-Lev work-stealing executor; a single-threaded determinism mode (needed because the existing deterministic-replay tests require it, and bit-identical output across SYCL-GPU and native-CPU is **not achievable** — FMA contraction and reduction-order differences mean the correct contract is bit-determinism *within* a backend, tolerance-comparison *across* backends).

### 4.4 Concrete call-site changes

`ecs/schedule.hpp` (`System::emit` retypes from `SushiRuntime::API::Graph&` to `Exec::Graph&`; the `graph.add(...)` call becomes `graph.add_parallel(NodeDesc{...}, kernel)` with the kernel lambda itself unchanged), `ecs/chunk.hpp` (`Column::data` becomes `Exec::Buffer<std::byte>`), `ecs/world.hpp` and `ecs/archetype.hpp` (constructor/member retype to `Exec::Context&`), `loop/app.hpp` (owns backend construction), `core/types.hpp` (gains a `Math::{sqrt,fmod,floor}` shim resolving to `sycl::*` under SYCL builds and `std::*` otherwise — this is already the file's documented job as "the single alias point"). Four `sycl::id<1>` kernel signatures convert to `std::size_t`. `audio/accelerator_sycl.hpp` needs **no change** — already correctly isolated.

**Net effect: SushiRuntime's blast radius across the engine collapses from 52 files touching it to roughly 3.** This alone discharges the standing project guidance that "SushiRuntime API is unstable — keep call sites thin/isolated," independent of any platform work.

### 4.5 Milestones

| Code | Delivers | Size |
|---|---|---|
| **RUNTIME-PORT0** | Extract the `Exec` seam behind the existing SYCL path, zero behavior change. Ships alone, standalone value. Exit: `sandbox` exits 0, `compile_count==1`, every example/test builds, byte-identical output logs. | Medium |
...
Also load-bearing: **every one of the 13 compute passes that hand-issue barriers declares zero attachments** — no pass mixes a rendering scope with manual barriers, which is exactly the invariant that makes a clean Metal encoder mapping possible. Enforce it as a debug-build assertion going forward, don't just rely on it holding by convention.
...
- **`show_in_explorer`/`open_with_default_app`** (`editor/ui/editor_panels.cpp`) are the representative case for the whole "Windows-only for now" pattern: replace `std::system()`-based shell calls (a real command-injection/quoting hazard on the existing code) with `SDL_OpenURL()` for "open with default app" (one call, correct on all five SDL-supported platforms already linked) and `posix_spawn`-based platform branches for "reveal in file manager" (no cross-platform primitive exists for this one). The same fix collapses the three scattered `#ifdef _WIN32` path-resolution forks onto `SDL_GetPrefPath()`.
...
1. **Immediately, in parallel, on Windows/Linux only:** `RUNTIME-PORT0` (Exec seam extraction) and `RHI0` (golden-image + trace harness). Neither touches behavior; both are prerequisites for everything else and have clean pass/fail oracles.
2. **Next:** `RUNTIME-PORT1` (native Exec backend, Linux-first control) and `RHI1–RHI3` (vocabulary → command list → device) can proceed in parallel — they don't share files. `PLATFORM0` (player split) can also start in parallel; its main technical risk (`present_scene_view()`) is renderer-adjacent but not RHI-abstraction-dependent, so it doesn't need to wait for RHI1–3.
...
5. **Once RHI6 (Android) has proven the hard portability bugs with good tooling:** `RHI7–8` (Metal). In parallel, `RUNTIME-PORT5` (iOS Exec) and `PLATFORM6` (macOS libs/CI) can proceed since they don't depend on Metal being finished, only on the version-floor work (RHI5) and the native Exec backend (RUNTIME-PORT2/5).
...
| O(N²) DAG compile stalls the native Exec backend at scale | 1 | High | Last-writer/reader-set linear construction (mirrors SushiRuntime's own tracker design) |
| Android/PLATFORM4 silently assumes RUNTIME-PORT4 is done when it isn't (or vice versa) | 1+3 | High | Make the stub-sim-vs-full-Exec decision explicit at PLATFORM4 kickoff, in writing, not by default |
...
1. **`RUNTIME-PORT0`** — extract the `Exec` seam behind the existing SYCL path, zero behavior change. Self-contained, has a pass/fail oracle (`sandbox`), and immediately collapses SushiRuntime's engine-wide blast radius from 52 files to ~3.
