...
layout(push_constant) uniform Push
{
    ...
    vec4 sun_radiance;   // rgb sun colour * intensity; w = lit/ambient: <0 unlit, >=0 lit (value = IBL ambient scale)
} pc;
...
void main()
{
    ...
    // Lit particles (the true-alpha bucket, e.g. smoke) receive the sun, the clustered punctual
    // lights, and the SH ambient. The additive and host-billboard buckets pass a negative flag and
    // stay emissive. The sprite is shaded as a camera-facing hemisphere: the offset gives a
    // spherical normal, so a light rakes across the puff.
    if (pc.sun_radiance.w >= 0.0)
    {
        ...
        // Sun: world-space directional, so no camera-relative conversion.
        float n_dot_l = max(dot(normal, pc.sun_direction.xyz), 0.0);
        vec3 sun = pc.sun_radiance.rgb * n_dot_l;
        ...
        rgb = in_color.rgb * (sun + punctual + ambient);
    }
