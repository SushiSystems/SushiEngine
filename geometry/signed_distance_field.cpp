...
namespace SushiEngine
{
    namespace Geometry
    {
                                                            ...
                                                            std::int32_t resolution)
        {
            ...
            float bounds_min[3];
            float bounds_max[3];
            if (!compute_bounds(mesh, bounds_min, bounds_max))
            {
                return brick;
            }
            Vec3 aabb_min{bounds_min[0], bounds_min[1], bounds_min[2]};
            Vec3 aabb_max{bounds_max[0], bounds_max[1], bounds_max[2]};
            ...
            for (std::size_t t = 0; t < triangle_count; ++t)
            {
                ...
                float p1[3];
                float p2[3];
                mesh.read_position(i0, p0);
                mesh.read_position(i1, p1);
                mesh.read_position(i2, p2);
