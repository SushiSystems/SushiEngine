...
TEST(Integration_NetClientServer, ReconciledClientConvergesWithRealCommand)
{
    ...
    const Vec3 expected = baseline_world.get<Position>(baseline_player).v;
    const Vec3 actual = client_world.get<Position>(client_player).v;
...
TEST(Integration_NetClientServer, DeterministicSpawnIdMatchesAcrossClientAndServer)
{
    ...
    const Vec3 spawn_position{Scalar(1), Scalar(2), Scalar(3)};
