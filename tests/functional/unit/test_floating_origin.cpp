...
// Unit_FloatingOrigin: WorldVector3 <-> FloatingOriginVector3 round-tripping. Planet-scale
...
{
    const FloatingOriginVector3 origin = to_floating_origin(WorldVector3{0, 0, 0}, SECTOR_SIZE);

    EXPECT_EQ(origin.sector, (SectorCoord{0, 0, 0}));
    EXPECT_TRUE(Harness::approx_equal(origin.local, Vector3{0, 0, 0}, Scalar(1e-6)));
}
...
{
    const WorldVector3 original{6'378'137.0, -1'234'567.5, 42.0};
...
TEST(Unit_FloatingOrigin, NegativeCoordinatesFloorTowardLowerSector)
{
    ...
    const FloatingOriginVector3 decomposed =
        to_floating_origin(WorldVector3{-1.0, 0.0, 0.0}, SECTOR_SIZE);
