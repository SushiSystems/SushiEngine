...
/**
 * @file audio_vorbis_demo.cpp
 * @brief The Vorbis codec (compressed music/dialogue) round-trip, end to end.
 *
 * Encodes a stereo tone to the bank's length-framed Vorbis packet stream, decodes it back
 * through @ref VorbisCodec chunk by chunk (the same @ref IAudioCodec seam the bank drives), and
 * checks the round-trip: far smaller than float, and the decoded signal correlates strongly
 * with the source (Vorbis is lossy). Also exercises the bank factory path. Exits 0 on success.
 */
...
int main()
{
    ...
    {
        const float s = 0.5f * static_cast<float>(std::sin(
                                   2.0 * 3.14159265358979323846 * frequency * f / sample_rate));
        source[static_cast<std::size_t>(f * channels + 0)] = s;
        source[static_cast<std::size_t>(f * channels + 1)] = s;
    }

    std::vector<std::uint8_t> encoded;
    if (!VorbisCodec::encode(source.data(), frames, channels, sample_rate, 0.4f, encoded))
    {
        std::fprintf(stderr, "audio_vorbis_demo FAILED: encode error\n");
        return 1;
    }
    ...
    std::printf("vorbis encode: %zu float bytes -> %zu packet bytes (%.1fx smaller)\n", source_bytes,
                encoded.size(), ratio);

    VorbisCodec codec(channels, sample_rate);
    std::vector<float> decoded;
    const int chunk_frames = 1024;
    std::vector<float> scratch(static_cast<std::size_t>(chunk_frames * channels), 0.0f);
    int offset = 0;
    int guard = 0;
    while (offset < static_cast<int>(encoded.size()) && guard++ < 100000)
    {
        int consumed = 0;
    ...
    std::printf("vorbis decode: %d frames back (source %d frames)\n", decoded_frames, frames);
    if (decoded_frames < frames / 2)
    {
        std::fprintf(stderr, "audio_vorbis_demo FAILED: too few frames decoded\n");
        return 1;
    }
    ...
    std::printf("vorbis round-trip correlation=%.4f\n", best_correlation);
    if (!(best_correlation > 0.9))
    {
        std::fprintf(stderr, "audio_vorbis_demo FAILED: weak correlation %.4f\n", best_correlation);
        return 1;
    }

    register_vorbis_codec();
    std::unique_ptr<IAudioCodec> bank_codec =
        make_codec(AudioCodecKind::Vorbis, channels, sample_rate);
    ...
    std::printf("bank make_codec(Vorbis) decoded %d frames\n", bank_frames);
    if (bank_frames < frames / 2)
    {
        std::fprintf(stderr, "audio_vorbis_demo FAILED: bank path decoded too few frames\n");
        return 1;
    }

    std::printf("audio_vorbis_demo OK\n");
    return 0;
}
