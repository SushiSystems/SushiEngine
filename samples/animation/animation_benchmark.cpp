...
int main()
{
    ...
    {
        skeleton_description.joints[j].name = "joint_" + std::to_string(j);
        skeleton_description.joints[j].parent = j == 0 ? -1 : static_cast<int>(j - 1);
        skeleton_description.joints[j].bind_translation = Vector3f{0.0f, 0.1f, 0.0f};
    }
    std::vector<std::byte> skeleton_blob;
    if (!build_skeleton_blob(skeleton_description, skeleton_blob))
    {
    ...
    clip_description.sample_rate = 30.0f;
    clip_description.translations.resize(static_cast<std::size_t>(FRAMES) * JOINTS);
    clip_description.rotations.resize(static_cast<std::size_t>(FRAMES) * JOINTS);
    clip_description.scales.resize(static_cast<std::size_t>(FRAMES) * JOINTS);
    for (std::uint32_t f = 0; f < FRAMES; ++f)
    {
        const float t = static_cast<float>(f) / clip_description.sample_rate;
        for (std::uint32_t j = 0; j < JOINTS; ++j)
        {
            const std::size_t i = static_cast<std::size_t>(f) * JOINTS + j;
            clip_description.scales[i] = Vector3f{1.0f, 1.0f, 1.0f};
            clip_description.translations[i] = Vector3f{0.0f, 0.1f, 0.0f};
            const float angle = 0.35f * std::sin(2.0f * 3.14159265f * 0.5f * t + j * 0.15f);
            clip_description.rotations[i] = axis_angle(0.0f, 0.0f, 1.0f, angle);
        }
    }

    std::vector<std::byte> raw_blob;
    build_clip_blob(clip_description, raw_blob);
    std::vector<std::byte> compressed_blob;
    if (!compress_clip(clip_description, 0.002f, compressed_blob))
    {
    ...
    std::printf("  clip (%.0f s, %u joints): raw %.1f KB -> compressed %.1f KB (%.1fx)\n",
                FRAMES / clip_description.sample_rate, JOINTS, raw_bytes / 1024.0,
