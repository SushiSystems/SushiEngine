/**************************************************************************/
/* ground_shadow_resolve_pass.cpp                                        */
/**************************************************************************/
...
namespace SushiEngine
{
    namespace Render
    {
        namespace Passes
        {
            GroundShadowResolvePass::GroundShadowResolvePass(
            ...
            GroundShadowResolvePass::~GroundShadowResolvePass() { destroy_pipeline(); }

            void GroundShadowResolvePass::create_pipeline()
            {
                pipeline_ = pipelines_.create(fullscreen_pipeline_desc(
                    layout_.pipeline_layout(), shaders_.module("fullscreen.vert"),
                    shaders_.module("ground_shadow_resolve.frag"), Frame::HDR_FORMAT));
            }

            void GroundShadowResolvePass::destroy_pipeline()
            {
            ...
            void GroundShadowResolvePass::rebuild_pipelines()
            {
                destroy_pipeline();
                create_pipeline();
            }

            void GroundShadowResolvePass::register_pass(Graph::RenderGraph& graph,
                                                         const Frame::FrameContext& frame)
            {
                if (!frame.targets.ground_shadow.valid() ||
                    !frame.targets.ground_shadow_resolved.valid())
                    return;

                graph.add_pass(
                    "ground shadow resolve",
                    [&](Graph::RenderPassBuilder& builder)
                    {
                        builder.color_attachment(0, frame.targets.ground_shadow_resolved,
                                                 Graph::AttachmentLoad::Discard);
                        builder.read(frame.targets.ground_shadow,
                    ...
                    [this, &frame](VkCommandBuffer cmd, const Graph::PassContext& context)
                    {
                        ...
                        writer.image(1, context.sampled_view(frame.targets.ground_shadow),
                                    sampler);
