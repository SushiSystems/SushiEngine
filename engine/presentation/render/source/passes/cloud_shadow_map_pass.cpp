...
namespace SushiEngine
{
    namespace Render
    {
        namespace Passes
        {
            namespace
            {
                constexpr VkFormat MAP_FORMAT = VK_FORMAT_R32_SFLOAT;
            ...
            CloudShadowMapPass::CloudShadowMapPass(Vulkan::VulkanDevice& device,
            ...
            {
                static_assert(RESOLUTION % AMORTIZE_FRAMES == 0,
                             "the map must divide evenly across the amortization window");
                ...
                Vulkan::check(vkCreateDescriptorSetLayout(device_.device(), &layout_info, nullptr,
                                                          &set_layout_),
                              "vkCreateDescriptorSetLayout(cloud shadow map)");
                ...
                Vulkan::check(vkCreatePipelineLayout(device_.device(), &pipeline_info, nullptr,
                                                     &pipeline_layout_),
                              "vkCreatePipelineLayout(cloud shadow map)");
            ...
            CloudShadowMapPass::~CloudShadowMapPass()
            {
                destroy_pipeline();
                destroy_map();
            ...
            void CloudShadowMapPass::create_pipeline()
            {
                pipeline_ = pipelines_.create_compute(pipeline_layout_,
                                                      shaders_.module("cloud_shadow_map.comp"));
            }

            void CloudShadowMapPass::destroy_pipeline()
            {
            ...
            void CloudShadowMapPass::rebuild_pipelines()
            {
                destroy_pipeline();
                create_pipeline();
            ...
            void CloudShadowMapPass::create_map()
            {
                VkImageCreateInfo image_info{};
                image_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
                image_info.imageType = VK_IMAGE_TYPE_2D;
                image_info.format = MAP_FORMAT;
                ...
                Vulkan::check(vmaCreateImage(device_.allocator(), &image_info, &alloc, &map_.image,
                                             &map_.allocation, nullptr),
                              "vmaCreateImage(cloud shadow map)");

                VkImageViewCreateInfo view_info{};
                view_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
                view_info.image = map_.image;
                view_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
                view_info.format = MAP_FORMAT;
                view_info.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
                view_info.subresourceRange.levelCount = 1;
                view_info.subresourceRange.layerCount = 1;
                Vulkan::check(vkCreateImageView(device_.device(), &view_info, nullptr, &map_.view),
                              "vkCreateImageView(cloud shadow map)");
            }

            void CloudShadowMapPass::destroy_map()
            {
                if (map_.view != VK_NULL_HANDLE)
                    vkDestroyImageView(device_.device(), map_.view, nullptr);
                if (map_.image != VK_NULL_HANDLE)
                    vmaDestroyImage(device_.allocator(), map_.image, map_.allocation);
                map_.view = VK_NULL_HANDLE;
                map_.image = VK_NULL_HANDLE;
                map_.allocation = VK_NULL_HANDLE;
            }

            void CloudShadowMapPass::register_pass(Graph::RenderGraph& graph,
                                                    const Frame::FrameContext& frame)
            {
                ...
                // advances, so a fixed row group refreshes every frame instead, cycling
                // the whole map across AMORTIZE_FRAMES frames regardless of what else
                ...
                graph.add_pass(
                    ...
                    {
                        // The map is pass-owned and barriered by hand below, exactly
                        // like the T3 field and the light volume; the one graph
                        // resource is the scene uniform block the bake reads the sun
                        // direction and cloud band from.
                     ...
                     first_bake](VkCommandBuffer command, const Graph::PassContext& context)
                    {
                        ...
                        writer.storage_image(0, map_.view);
                        ...
                        vkCmdPushConstants(command, pipeline_layout_, VK_SHADER_STAGE_COMPUTE_BIT,
                                           0, sizeof(Push), &push);
                        vkCmdDispatch(command, groups(RESOLUTION), groups(row_count), 1);

                        // Readable by every shadow consumer's fragment stage: sky.frag's
                        // ground march and pbr.frag's mesh shading alike.
                        transition(command, map_.image, VK_IMAGE_LAYOUT_GENERAL,
